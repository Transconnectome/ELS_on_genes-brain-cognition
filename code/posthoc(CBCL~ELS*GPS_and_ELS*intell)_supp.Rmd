...
## ======== glm function ======== ##
## ======== glm function ======== ##
glm_interactionELS_ELS_clinical_with_intel <- function(data, target, gps){
  intel_list <- c('nihtbx_totalcomp_uncorrected')
  final_result <- data.frame()
  
  for (intel in intel_list){
    fact_COV_LIST <- c(intel,'age','sex' ,'high.educ', 'income' ,'factor(married)','factor(abcd_site)', 'BMI', gps,'interaction_a')
    ...
    for (i in 3:5){
      fact_COV_LIST <- append(fact_COV_LIST,paste0(fact_COV_LIST[i],':',gps))
    }
...
glm_interactionIntel_ELS_clinical_with_intel <- function(data, target, gps){
  intel_list <- c('nihtbx_totalcomp_uncorrected')
  final_result <- data.frame()
  
  for (intel in intel_list){
    #fact_COV_LIST <- c(intel,'age','sex' ,'high.educ', 'income' ,'factor(married)','factor(abcd_site)', 'BMI', gps,'interaction_a','interaction_b')
    fact_COV_LIST <- c(intel,'age','sex' ,'high.educ', 'income' ,'factor(married)','factor(abcd_site)', 'BMI', gps,'interaction_b')
    X_independent <- target
    interaction_a <- data[,target] * data[,gps]
    interaction_b <- data[,target] * data[,intel]
    data <- cbind(data, interaction_a)
    ...
    for (CBCL_target in CBCL_list){
      ...
      glm.beta.gps <- rbind.data.frame(glm.beta.gps, cff[34,1])
      glm.beta.intel <- rbind.data.frame(glm.beta.intel, cff[3,1])
      glm.beta.interaction_b <- rbind.data.frame(glm.beta.interaction_b,cff[35,1])
      glm.p <- rbind.data.frame(glm.p, cff[2,4])
      glm.p.gps <- rbind.data.frame(glm.p.gps, cff[34,4])
      glm.p.intel <- rbind.data.frame(glm.p.intel, cff[3,4])
      glm.p.interaction_b <- rbind.data.frame(glm.p.interaction_b, cff[35,4])
      # VIF test
      #VIF = car::vif(glm.re)
      #PATH = paste0('~/Desktop/COGGPS_PAPER/only_EU/posthoc/CBCL_EAGPS_with_TOTAL_interaction_',CBCL_target,'_vif.csv')
      #write.csv(VIF, PATH, row.names=FALSE)
...
result_interactionELS <- glm_interactionELS_ELS_clinical_with_intel(final_phenotype_for_MM_european_CBCL,'Ab', gps='EA') 
result_interactionIntel <- glm_interactionIntel_ELS_clinical_with_intel(final_phenotype_for_MM_european_CBCL,'Ab', gps='EA') 
