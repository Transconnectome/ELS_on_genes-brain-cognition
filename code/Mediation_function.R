CP_Mediation <- function(nihtbx,n_boot){
  ...
  col_list<-colnames(CP_final)
  for (i in 1:length(col_list)){
    if (col_list[i] == nihtbx){
      names(CP_final)[i] <- 'nihtbx'
    }  
  }
  ...
  #result
  CP_fit <- lavaan::sem(CPmodel, data = CP_final,fixed.x = TRUE,std.lv = TRUE,se = 'bootstrap',bootstrap=n_boot,parallel = c("multicore"))
...
EA_Mediation <- function(nihtbx,n_boot){
  ...
  col_list<-colnames(EA_final)
  for (i in 1:length(col_list)){
    if (col_list[i] == nihtbx){
      names(EA_final)[i] <- 'nihtbx'
    }  
  }
  ...
  #save.image('MM_cryst_with_3PC.RData')
  #load('MM_cryst_with_3PC.RData')
  EA_fit <- lavaan::sem(EAmodel, data = EA_final,fixed.x = TRUE,std.lv = TRUE,se = 'bootstrap',bootstrap=n_boot,parallel = c("multicore"))
