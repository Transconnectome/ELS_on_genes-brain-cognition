...
###data prep
connectome<-read.csv(file ='con_aparc_fa.csv')
...
for (j in 1:2){
  for (gps in GPS_LIST[[j]]){
    ...
    FILE_NAME <- paste0('only_EU/GLM_con(fa)_cov_', gps, '_prsice2_corrected.csv')
...
brainList<-function(target){
  ...
  print(nrow(brain_list))
  #file_name<-paste0('',deparse(substitute(target)),'_brain_list.csv')
  
  #write.csv(brain_list,file_name,row.names = F,quote = F)
}

CP_related<-read.csv('only_EU/GLM_con(fa)_cov_CP_prsice2_corrected.csv')
EA_related<-read.csv('only_EU/GLM_con(fa)_cov_EA_prsice2_corrected.csv')
